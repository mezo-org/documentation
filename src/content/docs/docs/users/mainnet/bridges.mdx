...
| **MUSD** | Ethereum, Base | MUSD | Bridge as little as 0.01 MUSD between Mezo, Base, and Ethereum. Bridge powered by Wormhole. |
| **MEZO** | Ethereum, Base, BSC, Solana | MEZO | Bridge powered by Wormhole NTT. See the [MEZO Bridge guide](/docs/users/mezo/mezo-bridge) for details. |
...
-   **Wormhole Bridge:** A third-party bridge used for MUSD withdrawals to Ethereum and [MEZO token transfers](/docs/users/mezo/mezo-bridge) to Ethereum, Base, BSC, and Solana.
...
### Native Bridge's Fee Structure (v2)
...
- BTC-based tokens (BTC, mcbBTC, mFBTC, mSolvBTC, mswBTC, mxSolvBTC): 0.0002
- USD-based tokens bridged via Native Bridge (mUSDC, mUSDT, mDAI, mUSDe): 20
- mT tokens: 1,500

Fees

- Mezo Bridge Service Fee: ≈ $3 USD (computed in the target tokent)
- Network Fee (Gas): Paid in BTC, varies with network conditions

Example:

Withdrawing 1,000 USDC → $3 Service Fee + variable Gas Fee → receive ~997 USDC.
...
- Network Fee (Gas): Paid in BTC, varies
- Mezo Bridge Service Fee: ≈ 0.000027 BTC
- tBTC Bridge Service Fee: 0.2% (after Mezo Bridge Service Fee)
- tBTC Bridge Maximum Network Fee: up to 0.001 BTC (though usually < $1)

##### Example:

Withdraw amount: 1 BTC

Fees:
  - 0.000027 BTC (Mezo Bridge Service Fee)
  - 0.001999946 BTC (tBTC Bridge Service Fee)
  - variable tBTC Bridge Network Fee and Gas Fee

Received amount: ~0.997973 BTC.
...
A: Most transactions bridging funds to Ethereum via Native Bridge complete within 30-60 minutes, depending on network conditions and validator attestation speed. Withdrawals routed via Wormhole Bridge typically take around 5 minutes to process. Transactions withdrawing funds to Bitcoin usually take a couple of hours to finalize.
...
A: For Ethereum, the bridge system handles the final delivery gas costs. For Bitcoin, the tBTC Bridge Network Fee covers the cost of the on-chain transaction.
