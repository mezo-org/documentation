...
The second splitter in the emissions hierarchy. Splits non-validator emissions between [staking gauges](#staking-gauge) (90%) and [non-staking gauges](#non-staking-gauge) (10%), the latter funding [Mezo Gauges](#mezo-gauge). Set by veBTC vote. See [Emissions Schedule](/docs/users/mezo/emissions).
...
A smart contract that receives and distributes rewards based on votes. The more voting weight a gauge receives, the larger its share of MEZO emissions for that epoch. Gauge types include [staking gauges](#staking-gauge), [veBTC boost gauges](#vebtc-boost-gauge), and [non-staking gauges](#non-staking-gauge) such as [Mezo Gauges](#mezo-gauge). See [Mezo Earn Glossary](/docs/users/mezo-earn/overview/glossary).
...
A [non-staking gauge](#non-staking-gauge) that veMEZO holders vote on to direct MEZO emissions to MUSD liquidity pools outside Mezo — Curve and Uniswap on Ethereum, and Aerodrome on Base. Funded from the non-staking branch of emissions (currently 8% of weekly emission rewards). See [Mezo Gauges](/docs/users/mezo-earn/vote/mezo-gauges).
...
A [gauge](#gauge) with no staking token, voted on with veMEZO. Instead of paying stakers on Mezo, its emissions are routed to a destination the gauge names — for [Mezo Gauges](#mezo-gauge), a liquidity pool on another chain. Voting on one is independent of veBTC gauge voting and of [boost](#boost) matching. See [Mezo Gauges](/docs/users/mezo-earn/vote/mezo-gauges).
...
Vote-escrowed MEZO. An NFT representing locked MEZO tokens that can [boost](#boost) veBTC voting power up to 5x, and that votes in its own right on [veBTC boost gauges](#vebtc-boost-gauge) and [Mezo Gauges](#mezo-gauge). It carries no governance weight of its own and cannot vote on [staking gauges](#staking-gauge) without veBTC. Max lock is 4 years. See [veMEZO Overview](/docs/users/mezo-earn/lock/vemezo).
